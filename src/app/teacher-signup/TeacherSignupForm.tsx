...
export function TeacherSignupForm() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await redeemTeacherCodeAction({ code, email, password });
      if (res.ok) {
  ...
  return (
    <form onSubmit={onSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <AuthField label="Email" icon="user" name="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="prof@ecole.fr" autoComplete="email" />
