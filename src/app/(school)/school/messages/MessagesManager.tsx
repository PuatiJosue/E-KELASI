...
export function MessagesManager({
  ...
  const [mode, setMode] = useState<"inbox" | "message" | "reminder">(startInReminder ? "reminder" : "inbox");
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button
          ...
          className={mode === "message" ? "ek-btn ek-btn-primary" : "ek-btn ek-btn-outline"}
          style={{ height: 36, fontSize: 12.5 }}
        >
          <Icon name="send" size={14} /> <T fr="Nouveau message" en="New message" />
        </button>
        <button
      ...
      {mode === "reminder" ? (
        ...
        <MessageComposer recipients={reminderRecipients} onSent={() => { setMode("inbox"); router.refresh(); }} />
      ) : (
...
function MessageComposer({ recipients, onSent }: { recipients: ReminderRecipient[]; onSent: () => void }) {
  const [pending, start] = useTransition();
