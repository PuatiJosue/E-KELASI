...
import { redeemTeacherCodeAction } from "@/app/(school)/school/teachers/actions";

export function SurveillantSignupForm() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await redeemTeacherCodeAction({ code, email, password, expectedRole: "surveillant" });
      if (res.ok) {
  ...
  return (
    <form onSubmit={onSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <AuthField label="Email" icon="user" name="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="surveillant@ecole.cd" autoComplete="email" />
