...
import type { PlatformVideo, PlatformParent } from "@/lib/platform-db";
...
export function VideosManager({ videos, parents }: { videos: PlatformVideo[]; parents: PlatformParent[] }) {
  ...
  const submit = () => {
    ...
    start(async () => {
      ...
      if (file) {
        if (file.size > MAX_VIDEO_BYTES) { setErr("La vidéo dépasse 50 Mo. Utilisez un lien pour les fichiers plus lourds."); return; }
        setUploading(true);
        try {
      ...
      const res = await addPlatformVideo({
        title, url: finalUrl, subject,
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div className="ek-card" style={{ padding: 18 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <label className="ek-btn ek-btn-outline" style={{ height: 36, fontSize: 12.5, cursor: "pointer" }}>
              ...
              <input ref={fileRef} type="file" accept="video/*" style={{ display: "none" }} onChange={(e) => setFileName(e.target.files?.[0]?.name ?? null)} />
            </label>
            {fileName && (
          ...
          <div style={{ borderTop: "1px solid var(--divider)", paddingTop: 12, marginTop: 2 }}>
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: target === "parents" ? 10 : 0 }}>
              <label style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, color: "var(--ink-2)", cursor: "pointer" }}>
                ...
                <T fr="Tous les parents" en="All parents" />
              </label>
              <label style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, color: "var(--ink-2)", cursor: "pointer" }}>
            ...
            {target === "parents" && (
              <div style={{ border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden" }}>
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher un parent (nom ou email)…" style={{ ...input, height: 36, border: "none", borderBottom: "1px solid var(--divider)", borderRadius: 0 }} />
                <div style={{ maxHeight: 220, overflowY: "auto" }}>
                  {parents.length === 0 ? (
                    <div style={{ padding: 14, fontSize: 12.5, color: "var(--ink-3)" }}><T fr="Aucun parent." en="No parent." /></div>
                    ...
                    filteredParents.map((p, i) => (
                      <label key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", cursor: "pointer", borderTop: i > 0 ? "1px solid var(--divider)" : "none", background: selected.has(p.id) ? "var(--brand-soft)" : "transparent" }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          {p.email && <div style={{ fontSize: 11, color: "var(--ink-3)" }}>{p.email}</div>}
                        </div>
                      </label>
                    ))
                  )}
        ...
        <button onClick={submit} disabled={busy} className="ek-btn ek-btn-primary" style={{ marginTop: 12, height: 38, fontSize: 13, opacity: busy ? 0.6 : 1 }}>
          <Icon name="plus" size={14} stroke={2.5} /> {uploading ? <T fr="Envoi…" en="Uploading…" /> : <T fr="Publier" en="Publish" />}
        </button>
      </div>
